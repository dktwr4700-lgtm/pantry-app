...
export default function App() {
  ...
  if (shareParams.get("mode") === "share" && shareParams.get("code")) {
    const itemsParam = shareParams.get("items");
    return (
      <ShareView
        code={shareParams.get("code")}
        fromPhone={shareParams.get("from") || ""}
        tripItemIds={itemsParam !== null ? itemsParam.split(",").filter(Boolean) : null}
      />
    );
  }
  ...
  // The needed item IDs at send time are baked into the link itself
  // (not just read from live data on first load) so the list this
  // link shows is locked to exactly what was needed when it was sent
  // — even if the same browser tab gets reused for an older link, or
  // items change on other devices afterward.
  const buildInteractiveLink = () => {
    ...
    params.set("items", neededIds.join(","));
...
// the main app uses.
function ShareView({ code, fromPhone, tripItemIds }) {
  useFonts();
  const [items, setItems] = useState(null); // null = loading
  const [justCompleted, setJustCompleted] = useState(false);
  const prevNeededRef = useRef(null);
  // Which item IDs belong to "this shopping trip". Prefer the list
  // baked directly into the URL when the link was generated — that's
  // exact and can't drift. Fall back to a snapshot on first load only
  // for older links that don't carry it.
  const tripItemIdsRef = useRef(
    tripItemIds && tripItemIds.length > 0 ? new Set(tripItemIds) : null
  );

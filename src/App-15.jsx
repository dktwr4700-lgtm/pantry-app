...
function ShareView({ code, fromPhone }) {
  ...
  // Show every item in each category — not just the still-needed ones —
  // so the shopper can scroll back and see/undo what they already
  // checked off, instead of items vanishing the moment they're bought.
  // Needed items float to the top of each category; bought ones sit
  // below, struck through.
                ...
                >
                  <div
                    style={{
                      ...
                      height: 24,
                      borderRadius: 8,
                      border: `2px solid ${item.needed ? "#DDD0B3" : green}`,
                      background: item.needed ? "transparent" : green,
                      color: "#fff",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label={item.needed ? "لسا ناقص — اضغط لو خلص عندك" : "عندك — اضغط لو صار ناقص"}
      >
        <span
          style={{
            width: 25,
            height: 25,
            borderRadius: 8,
            border: `2px solid ${item.needed ? COLORS.rust : COLORS.sage}`,
            background: item.needed ? "transparent" : COLORS.sage,
            color: COLORS.paper,
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            transition: "background 0.18s ease, transform 0.18s cubic-bezier(0.34, 1.56, 0.64, 1)",
            transform: item.needed ? "scale(0.94)" : "scale(1)",
          }}
        >
          {item.needed ? null : <Icon name="check" size={13} strokeWidth={3} color={COLORS.paper} />}
        </span>
      </button>
    </div>
  );

    ...
    >
      ...
      {tab === "pantry" && (
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "18px 20px" }}>
          ...
          >
            {[
              { icon: "📋", label: t("templatesBtn").split(" ")[0], onClick: () => setShowTemplates(true) },
              { icon: "🍳", label: "أطبخ اليوم", onClick: () => setShowRecipes(true) },
              { icon: "📸", label: "مسح ورقي", onClick: () => setShowOcrScan(true) },
              { icon: "👆", label: "إضافة سريعة", onClick: openQuickAdd },
            ].map((tool) => (
              <button
                key={tool.label}
      ...
      >
        {[
          { id: "pantry", icon: "🏠", label: "الرئيسية" },
          { id: "shopping", icon: "📝", label: "القوائم" },
          { id: "favorites", icon: "⭐", label: "المفضلة" },
          { id: "nutrition", icon: "🍽️", label: "تغذية" },
        ].map((nb) => {
          ...
          return (
            ...
            >
              <span style={{ fontSize: 19 }}>{nb.icon}</span>
              <span
                style={{
                  fontFamily: "'Cairo', sans-serif",
                  fontWeight: 700,
...
function ShareView({ code, fromPhone }) {
  ...
  // Progress is based on how many of the currently-visible categorized
  // items are left — simplest honest signal without extra bookkeeping.
  const groups = CATEGORIES.map((c) => ({
    ...c,
    items: neededItems.filter((i) => i.category === c.id),
  })).filter((g) => g.items.length > 0);
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label={item.needed ? "محتاجه" : "متوفر"}
      >
        <span
          style={{
            width: 24,
            ...
            background: item.needed ? COLORS.rust : "transparent",
            color: COLORS.paper,
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
